...
class Genome:
    ...
    def start(self):
        # Solution to find
        expected_string = np.array(['h', 'e', 'l', 'l', 'o', 'w', 'o', 'r', 'l', 'd'], dtype=np.str)
        best_outputs = []
        mean_outputs = []
        worst_outputs = []
        ...
        for i in range(self.pop_size[0]):
            new_pop.append([self.generation_fx() for i in range(self.pop_size[1])])
        new_population = np.array(new_pop, dtype=np.str)
...
def selection(pop, fitness, num_parents):
    ...
    parents = np.empty((num_parents, pop.shape[1]), dtype=np.str)
...
def reproduction(parents, offspring_size):
    offspring = np.empty(offspring_size, dtype=np.str)
    # The point at which reproduction takes place between two parents. Usually, it is a random point
    reproduction_point = np.random.randint(offspring_size[1]-1)
...
bit_string = np.array(['h', 'e', 'l', 'l', 'o', 'w', 'o', 'r', 'l', 'd'], dtype=np.str)
